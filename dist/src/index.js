...
import { DRAGONSWAP_V1_TEST_CONFIG } from './config/constants.js';
...
export default function ({ config }) {
    ...
    server.tool('list_pools', 'List available liquidity pools on DragonSwap V1.', listPoolsSchema.shape, async (args) => {
        try {
            const { limit } = args;
    ...
    server.tool('get_quote', 'Get a quote for swapping tokens on DragonSwap V1.', getQuoteSchema.shape, async (args) => {
        try {
    ...
    server.tool('execute_swap', 'Execute a swap on DragonSwap V1.', executeSwapSchema.shape, async (args) => {
        try {
            const { tokenIn, tokenOut, amountIn } = args;
    ...
    server.tool('wrap_sei', 'Wrap native SEI into wSEI.', wrapSeiSchema.shape, async (args) => {
        try {
            const { amount } = args;
